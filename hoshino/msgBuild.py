...
from hoshino.config import IMAGE_PATH, VOICE_PATH, NICKNAME
...
def image(
    ...
    if abspath:
        ...
        hoshino.logger.error(f'缺少图片资源：{img_name.absolute()}')
        return ""
    elif b64:
        ...
        if file.exists():
            ...
            hoshino.logger.warning(f"图片 {file.absolute()}缺失...")
            return ""
...
def record(voice_name: str, path: str = None) -> MessageSegment or str:
    ...
    if file.exists():
        ...
        hoshino.logger.warning(f"语音{file.absolute()}缺失...")
        return ""
